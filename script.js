...
function mainFunc(i, j) {
    ...
    if (mapCell[i][j][2] === 9){ // если тип клетки - ближний очиститель
        if (mapCell[i][j][4] === 0){
            ...
            // механики
            if (mapCell[i][j][1] >= energyToMeleeClean) { // если хватает энергии для очистки
                mapCell[i][j][1] -= energyToMeleeClean; // тратим энергию за очистку
                for (let a = -1; a < 4; a++) { // действуем в квадрате 5x5 вокруг клетки-очистителя
                    for (let b = -1; b < 4; b++) {
                        if (i-1+a >= 0 && i-1+a < mapH && j-1+a >= 0 && j-1+a < mapH && mapCell[i-1+a][j-1+a][2] != 0 && mapCell[i-1+a][j-1+a][3] != mapCell[i][j][3]) { // если не выходит за карту, очищаемая клетка не пустая и очищаемая клетка другой фракции
                            mapTable.rows[i-1+a].cells[j-1+a].textContent = 'O'; // отображаем очищение
    ...
    if (mapCell[i][j][2] === 10){ // если тип клетки - дальний очиститель
        if (mapCell[i][j][4] === 0){
            ...
            // механики
            if (mapCell[i][j][1] >= energyToDistantCombat) { // если хватает энергии для очистки
                let attackDirect = rand(0, 3);
                if (attackDirect === 0) { // очищаем налево
                    let stopper = 0;
                    let distant = 1;
                    while (stopper === 0) {
                        if (j - distant < 0) { // если клетка за границей карты - просто прерываем цикл, выстрела не будет
                        ...
                        mapCell[i][j][1] -= energyToDistantCombat; // вычитаем энергию за выстрел
                        if (mapCell[i][j - distant][3] != mapCell[i][j][3] && mapCell[i][j - distant][2] != 0) { // если фракция не наша и клетка не пустая
                            mapTable.rows[i].cells[j - distant].textContent = 'O'; // рисуем очищение
                ...
                if (attackDirect === 1) { // очищаем вперед
                    let stopper = 0;
                    let distant = 1;
                    while (stopper === 0) {
                        if (i - distant < 0) { // если клетка за границей карты - просто прерываем цикл, выстрела не будет
                        ...
                        mapCell[i][j][1] -= energyToDistantCombat; // вычитаем энергию за выстрел
                        if (mapCell[i - distant][j][3] != mapCell[i][j][3] && mapCell[i][j - distant][2] != 0) { // если фракция не наша и клетка не пустая
                            mapTable.rows[i - distant].cells[j].textContent = 'O'; // рисуем очищение
                ...
                if (attackDirect === 2) { // очищаем направо
                    let stopper = 0;
                    let distant = 1;
                    while (stopper === 0) {
                        if (j + distant > mapW) { // если клетка за границей карты - просто прерываем цикл, выстрела не будет
                ...
                if (attackDirect === 3) { // очищаем вниз
                    let stopper = 0;
                    let distant = 1;
                    while (stopper === 0) {
                        if (i + distant > mapH) { // если клетка за границей карты - просто прерываем цикл, выстрела не будет
