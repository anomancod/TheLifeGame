...
const numInRatioForExp = 9; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для экспов
const numInRatioForQua = 9; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для качественников
const numInRatioForNom = 9; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для кочевников
...
const period = setInterval(() => {
    ...
    for (let i = 0; i < mapH; i++) { // проходимся по всем элементам карты
        for (let j = 0; j < mapW; j++) {
            ...
            if (mapCell[i][j][2] != 0 && mapCell[i][j][2] != 1 && mapCell[i][j][2] != 1 && mapCell[i][j][2] != 1) {
