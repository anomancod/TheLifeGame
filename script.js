...
function mainGenome(i, j){
    ...
    if(gen[3] < 105 || gen[5] < 105){ // если хотя бы одно из условий задано
        if(gen[3] < 105){ // если первое условие задано
            let resFirstIf = ifFunc[Math.ceil(gen[3]/7)](i, j, gen[4]); // вызываем функций по нужному индексу и передаем коорд. с параметром и принимаем результат (0 - не выпол., 1 - выпол.)
        }
        if(gen[5] < 105){ // если второе условие задано
            let resSecondIf = ifFunc[Math.ceil(gen[3]/7)](i, j, gen[5]); // вызываем функций по нужному индексу и передаем коорд. с параметром и принимаем результат (0 - не выпол., 1 - выпол.)
        }
