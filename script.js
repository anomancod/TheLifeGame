...
const maxEnergyTrans = 10000; // макс. энергия стебля (транспортной клетки) {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
const maxEnergySprout = 5000; // макс. энергия отростка {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
const maxEnergyWar = 500; // макс. энергия боевых клеток {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}

const startSproutEnergy = 2000; // нач. энергия отростка {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
...
const numInRatioForExp = 10; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для экспов
const numInRatioForQua = 10; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для качественников
const numInRatioForNom = 10; // число для соотношения: если отростков (с пустыми клетками вокруг) меньше [кол-во всех клеток] / (это число) для кочевников

const turnsNoChangeStartTacticExp = 7; // число ходов без смены начальной тактики ДЛЯ ЭКСПОВ {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
const turnsNoChangeStartTacticQua = 4; // число ходов без смены начальной тактики ДЛЯ КАЧЕСТВЕННИКОВ {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
const turnsNoChangeStartTacticNom = 5; // число ходов без смены начальной тактики ДЛЯ КОЧЕВНИКОВ {!!! ТЕСТОВОЕ ЗНАЧЕНИЕ !!!}
...
const period = setInterval(() => {
    ...
    for (let i = 0; i < mapH; i++) { // проходимся по всем элементам карты
        for (let j = 0; j < mapW; j++) {
            // если клетка не пустая, не отросток, не стебель и не семя
            if (mapCell[i][j][2] != 0 && mapCell[i][j][2] != 1 && mapCell[i][j][2] != 2 && mapCell[i][j][2] != 6) {
